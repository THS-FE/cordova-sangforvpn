...
/*! @brief
 ...
 @discussion
 SDK初始化以及进行AuthInfo认证
 先开始vpn初始化流程，初始化成功之后自动开始AuthInfo认证流程
 ...
 @param authInfo 认证信息
 */
- (void)startAuthInfoLogin:(VPNMode)mode
                vpnAddress:(NSURL *)url
                      type:(NSString *)type
...
/*! @brief
 ...
 用于打包vpn的日志文件，
 同步方法，请自行做异步处理
 @param path 要打包到的文件路径，默认为 document下的SFSDK.log.zip
 @return 空字符串为打包未成功， 否则为打包的绝对路径
 */
- (NSString *)packLogToPath:(nullable NSString *)path;

/*! @brief
 日志打包并上传到深信服的日志收集服务器
 
 @discussion
 用于打包vpn的日志文件并上传至深信服的日志收集服务器
 同步方法，请自行做异步处理
 @param url 预防日志平台升级导致需要重新切换url的场景，如无特殊说明，请传 nil
 @param error 上传过程中遇到的错误及原因
 @return 空字符串提交未成功， 否则为提交的单号和时间，中间以”$“分割
 */
- (NSString *)uploadLogsToSangforUrl:(nullable NSString *)url error:(NSError **)error;
@end
